...
export default function MyLessonsPage() {
  ...
  useEffect(() => {
    async function fetchMyLessons() {
      try {
        // 1. Get email securely from your session action
        ...
        // 2. Sync the email into the browser's URL bar if it's missing or different
        const currentEmailParam = searchParams.get('email');
        if (currentEmailParam !== email) {
          router.replace(`/dashboard/my-lessons?email=${encodeURIComponent(email)}`, { scroll: false });
        }

        // 3. Fetch data from your backend API matching the custom query parameter
        const response = await fetch(`http://localhost:5000/dashboard/my-lessons?email=${encodeURIComponent(email)}`);
        ...
        console.error("Failed fetching user's custom lessons:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (lessons.length === 0) {
    return (
      <div className="text-center py-16 max-w-xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-slate-100 mb-2">You haven&apos;t posted any lessons yet</h2>
        <p className="text-slate-400 mb-6">Lessons linked with <span className="text-blue-400 font-mono">{userEmail}</span> return zero matching results.</p>
    ...
    <div className="max-w-7xl mx-auto p-6 md:p-10 space-y-8">
      <header className="border-b border-slate-800 pb-6">
        ...
        <p className="text-sm text-slate-400 mt-2">
          Manage, review, and track performance of lessons published under your profile matching <span className="text-blue-400 font-mono">{userEmail}</span>.
        </p>
      </header>
          ...
          >
            <CardContent className="p-5 flex flex-col gap-4">
              <div className="flex flex-wrap gap-1.5">
                ...
                <span className="px-2.5 py-0.5 text-xs font-medium bg-emerald-950/40 text-emerald-400 border border-emerald-100/10 rounded-full ml-auto">
            ...
            <CardFooter className="p-5 border-t border-slate-800/80 flex flex-col gap-4 bg-slate-900/30">
              ...
              <Button 
                ...
                onClick={() => router.push(`/dashboard/lessons/${lesson._id || lesson.id}`)}
              >
                See Details
              </Button>
            </CardFooter>
